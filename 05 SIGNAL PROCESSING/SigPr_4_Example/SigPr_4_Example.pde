...
void draw() {
  ...
  for (int i=0; i<widthLim; i++) {
    for (int j=0; j<heightLim; j++) {
      ...
      float sinR = map(sin(radians(red(input[i][j])*10)+frameCount*0.5), -1, 1, 0, red(input[i][j]));
      float sinG = map(sin(radians(green(input[i][j])*20)+frameCount*0.5+HALF_PI), -1, 1, 0, green(input[i][j]));
      float sinB = map(sin(radians(blue(input[i][j])*40)+frameCount*0.5+PI), -1, 1, 0, blue(input[i][j]));
      output.set(i, j, color(sinR, sinG, sinB));
    }
  }
